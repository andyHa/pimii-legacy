...
String Parser::getCurrentString() {
    return tokenizer.getString(tokenizer.getCurrent()).toStdWString();
}
...
Node* Parser::definition() {
    ...
    param->setStringAttribute("name", getCurrentString());
    result->addChild("params", param);
    symbols->push_back(getCurrentString());
    ...
    if (tokenizer.getCurrent().type == TT_L_CURLY) {
        ...
        //TODO generateFunctionCode(result, brackets);
    }
...
void Parser::generateFunctionCode(Node* blk, bool expectBracet) {
    if (expectBracet) {
